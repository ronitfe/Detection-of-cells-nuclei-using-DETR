"""yolo-for-detection-of-bounding-boxes-tensorflow.ipynb

This piece of work is inspired by a [youtube video lecture](
https://www.youtube.com/watch?v=vaL1I2BD_xY)
 [YOLO](https://arxiv.org/pdf/1506.02640.pdf)).
  https://colab.research.google.com/drive/17KCXUluoADwbkHvikPoTjxEeOqZAu4il
...
from yolo_utils import (TRAIN_PATH, TEST_PATH, IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS,
                        store_bounding_boxes, get_grid_info)
...
logits = tf.layers.conv2d(net, filters=MAX_BB_CNT * 5 * 16, kernel_size=1, strides=1,
                          padding="same",
...
start_training = False  # using pretrained model as Kaggle server doesn't allow training time
# more than 6 hrs.
if not start_training:
    test_image_id = 50
    saver = tf.train.Saver()
    with tf.Session() as sess:
        saver.restore(sess, "pre-trained-model/model.ckpt")
        result = sess.run([processed_logits],
                          {X: np.reshape(test_images[test_image_id], [1, 384, 384, 3])})

# !unzip pre-trained-model/model.ckpt.meta.zip
...
if (start_training):
    ...
    for i in range(33000):
        ...
        if (i % 100 == 0):
            print(
                str(display_count) + " training loss(" + str(mask_count) + "): " + str(train_loss))
            display_count += 1
        mask_count = 0
    print("Done!")
